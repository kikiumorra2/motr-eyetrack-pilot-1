...
class associationMerger:
    def __init__(
        self,
        raw_data_divided_path,
        merged_association_path,
        lower_threshold: int,
        upper_threshold: int,
    ):
            ...
            mouse_data = [
                {
                    ...
                    # "expr_id": int(row["Experiment"]),
                    "expr_id": str(row["Experiment"]),
                    # "cond_id": int(row["Condition"]),
                    "cond_id": str(row["Condition"]),
                    # "para_nr": int(row["ItemId"]),
                    "para_nr": str(row["ItemId"]),
                    "word_nr": int(row["Index"]),
                    "word": str(row["Word"]),
                    "t": int(float(row["responseTime"])),
                    "x": int(float(row["mousePositionX"])),
                    "y": int(float(row["mousePositionY"])),
            ...
            if self.mouse_data[i + 1]["para_nr"] == self.mouse_data[i]["para_nr"]:
                if self.mouse_data[i + 1]["word_nr"] == self.mouse_data[i]["word_nr"]:
                    ...
                    merged_association_on_word = {
                        ...
                        "expr_id": self.mouse_data[i]["expr_id"],
                        "cond_id": self.mouse_data[i]["cond_id"],
                        "para_nr": self.mouse_data[i]["para_nr"],
                        "word_nr": self.mouse_data[i]["word_nr"],
                        "word": self.mouse_data[i]["word"],
                        "duration": fixed_time,
                        "start_t": self.mouse_data[i]["t"],
                        "end_t": self.mouse_data[i + 1]["t"],
                        "x_mean": round(mean(x_coordinates), 2),
                        "y_mean": round(mean(y_coordinates), 2),
    ...
    def write_out_all_merged_associations(self) -> None:
        ...
        with open(
            f"{self.out_data_path}/{self.out_data_merged_name}", "w", newline=""
        ) as out_csvfile:
            if self.associations:
                # to avoid errors given by mess data, we can manually type fieldnames here later.
                writer = csv.DictWriter(
                    out_csvfile, fieldnames=self.associations[0][0].keys()
                )
                writer.writeheader()
    ...
    def sort_associations_by_itemid(self) -> None:
        ...
        self.associations = sorted(
            # self.associations, key=lambda x: x[0]["para_nr"] if x else float("inf")
            self.associations,
            key=lambda x: x[0]["para_nr"] if x else "",
        )

    def _clear_noises_before_reading(self):
        for i in range(len(self.associations)):
            while self.associations[i] and (
                self.associations[i][0]["word_nr"] not in self.start_sent
            ):
    ...
    def write_out_denoise_merged_associations(self) -> None:
        ...
        with open(
            f"{self.out_data_path}/{self.out_data_merged_denoise_name}", "w", newline=""
        ) as out_csvfile:
            ...
            for item in self.associations:
                for association_on_word in item:
                    if (
                        ...
                        < association_on_word["duration"]
                        < self.threshold_upper
                        and association_on_word["word_nr"] != -1
                    ):
                        writer.writerow(association_on_word)
