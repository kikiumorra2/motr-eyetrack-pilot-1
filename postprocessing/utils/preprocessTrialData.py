...
class TrialDataPreprocessor:
    def __init__(self, raw_data_path: str, new_data_path: str, delim: str):
    ...
    def split_sentence_into_words(self) -> None:
        ...
        self.raw_data_df = self.raw_data_df.assign(
            text=self.raw_data_df["text"].str.split()
        ).explode("text")
        self.raw_data_df["word_nr"] = self.raw_data_df.groupby("item_id").cumcount()
        self.raw_data_df.to_csv(
            self.new_data_path / f"preprocessed_{self.raw_data_path.stem}.csv",
            index=False,
            float_format=None,
        )

    def filtered_new_df(self) -> None:
        df_new = self.raw_data_df[
            [
                # "experiment_id",
                "condition_id",
                "item_id",
        ...
        df_new.to_csv(
            self.new_data_path / f"filtered_preprocessed_{self.raw_data_path.stem}.csv",
            index=False,
        )
