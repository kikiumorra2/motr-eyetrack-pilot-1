...
class FileDivider:
    ...
    def divide_raw_file(self) -> None:
        ...
        for submission_id, group in grouped_data:
            group.to_csv(
                self.new_data_path / f"reader_{submission_id}.csv",
                index=False,
                float_format=None,
            )
    ...
    def _fill_nan_response_column(self) -> None:
        ...
        # The answer is recorded on the trial-summary row, after the trial's samples.
        # Back-fill it onto the samples within each participant x trial only, so that a
        # trial without an answer keeps NaN instead of inheriting the next trial's answer.
        self.raw_data_df["response"] = self.raw_data_df.groupby(
            ["submission_id", "ItemId"], sort=False
        )["response"].bfill()
        self.raw_data_df["ItemId"] = self.raw_data_df["ItemId"].ffill()
        ...
        self.raw_data_df["Experiment"] = self.raw_data_df["Experiment"].ffill()
        # self.raw_data_df["Experiment"] = (
        #     self.raw_data_df["Experiment"].astype(float).astype(int)
        # )
        self.raw_data_df["Condition"] = self.raw_data_df["Condition"].ffill()
        # self.raw_data_df["Condition"] = (
        #     self.raw_data_df["Condition"].astype(float).astype(int)
        # )
        self.raw_data_df["Index"] = self.raw_data_df["Index"].fillna(-100)
        ...
        self.raw_data_df["mousePositionX"] = self.raw_data_df["mousePositionX"].fillna(425)
        self.raw_data_df["mousePositionX"] = (
            self.raw_data_df["mousePositionX"].astype(float).astype(int)
        )
        self.raw_data_df["mousePositionY"] = self.raw_data_df["mousePositionY"].fillna(285)
        self.raw_data_df["mousePositionY"] = (
            self.raw_data_df["mousePositionY"].astype(float).astype(int)
        )

    def correct_motr_data(self):
        # List all CSV files in the directory
        divided_files = Path(self.new_data_path).glob("*.csv")
        corrected_divided_path = (
            Path(self.new_data_path).parent
            / f"corrected_{Path(self.new_data_path).name}"
        )
        ...
        for file_path in divided_files:
            ...
            if (df["mousePositionX"] < -1).any():
                ...
                combined_positions = (
                    negative_mouse_positions | only_to_correct_word_position
                )
                ...
                normal_mouse_positions = df["wordPositionLeft"].notnull() & df[
                    "wordPositionTop"
                ].notnull() & (
                    df["mousePositionX"]
                    <= df["wordPositionRight"] - df["wordPositionLeft"]
                ) | (
                    df["mousePositionY"]
                    <= df["wordPositionBottom"] - df["wordPositionTop"]
                )
                ...
                df.loc[normal_mouse_positions, "mousePositionX"] += df[
                    "wordPositionLeft"
                ]
                df.loc[normal_mouse_positions, "mousePositionY"] += df[
                    "wordPositionTop"
                ]
                df.loc[
                    combined_positions, ["wordPositionBottom", "wordPositionTop"]
                ] += 40
                ...
                for i in reversed(range(len(df))):  # Start from the last row
                    if pd.notnull(df.at[i, "wordPositionLeft"]):
                        ...
                        df.at[i, "mousePositionX"] += df.at[
                            last_valid_index, "wordPositionLeft"
                        ]
                        df.at[i, "mousePositionY"] += df.at[
                            last_valid_index, "wordPositionTop"
                        ]
        ...
        print(
            f"All files in '{self.new_data_path}' have been processed and corrected files are saved in '{corrected_divided_path}'."
        )
